---
title: "Model CPI - For Revenue- CFF3"
author: "UC Team"
format: html
editor: visual
---
...
CFF_3_Revenue_CPI<-merge(CFF_3_agg_revenue,CPI,by="Date")
CFF_3_Revenue_CPI
write.csv(CFF_3_Revenue_CPI,"CFF_3_Revenue_CPI.csv")
```
...
# Reading data
data <- fread("CFF_3_Revenue_CPI.csv", header = T, fill = TRUE)
#head(data)
summary(data)
...
***Therefore, we can conclude that*** because of the low p-value of the Anova test between model 1 and 2, we can conclude that CPI granger causes Revenue.

For every unit increase in **xh3**, the response variable that is **Revenue is estimated to increase by 12.51567** **units**, holding other predictors constant.
...
effect1 = 12.51567*(log(101.7) - log(100.7))
...
*For every unit increase in CPI, revenue will increase by 13%*
