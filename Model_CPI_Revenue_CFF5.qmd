---
title: "Model CPI - For Revenue- CFF5"
author: "UC Team"
format: html
editor: visual
---
...
CFF_5_agg_revenue
library("writexl")
write.csv(CFF_5_agg_revenue,"CFF_5_agg_revenue.csv")
...
CFF_5_Revenue_CPI<-merge(CFF_5_agg_revenue,CPI,by="Date")
CFF_5_Revenue_CPI
write.csv(CFF_5_Revenue_CPI,"CFF_5_Revenue_CPI.csv")
```
...
# Reading data
data <- fread("CFF_5_Revenue_CPI.csv", header = T, fill = TRUE)
#head(data)
summary(data)
...
The Revenue plot has an exponentially decreasing plot with spikes after 2021.
...
CPI follows an increasing curve after the dip after 2020.
...
***Therefore, we can conclude that*** CPI granger causes Revenue.

xh3 is statistically significant.

*For every unit increase in **xh3,** the response variable i.e Revenue is estimated to increase by **12.54754 units**, holding other predictors constant.*
...
effect1 = 12.54754*(log(115.1) - log(114.1))
...
*For every unit increase in CPI, revenue will increase by 12%*
