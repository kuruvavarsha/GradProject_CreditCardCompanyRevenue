---
title: "Unemployment Rate-Revenue -  CFF5 "
...
library("writexl")
  ...
  CFF_5_Unrate_revenue<-merge(CFF_5_agg_revenue,unemployment_rate, by ="Date")
  CFF_5_Unrate_revenue
  write.csv(CFF_5_Unrate_revenue,"CFF_5_Revenue_Unrate.csv")
```
...
# Reading data
data <- read.csv("CFF_5_Revenue_Unrate.csv", header = T, fill = TRUE)
#head(data)
summary(data)
...
The Revenue plot has an exponentially decreasing plot .
...
Unemployment Rate: xt <br>Revenue: yt <br>*Since both these series are nonstationary we have transformed them first before the Granger Causality test.* <br> Δxt = log(xt) − log (xt-1) <br> Δyt = log(𝑦t) − log (𝑦t-1) <br> xh = Δxt <br> yh = Δyt <br>
