---
title: "Personal Savings Rate vs Ratio CFF 3"
author: "UC Team"
format: html
editor: visual
---
...
CFF_3_Savings_ratio <- merge(CFF_3_agg_ratio,Savings , by = "Date")
CFF_3_Savings_ratio
write.csv(CFF_3_Savings_ratio,"CFF_3_Savings_ratio.csv")
```
...
#| include: false
# Reading data
data <- fread("CFF_3_Savings_ratio.csv", header = T, fill = TRUE)
#head(data)
#summary(data)
...
## Time series plot of ratio of actual Ratio to predicted Ratio - Variable Y
...
#head(data_final)
colnames(data_final) <- c("time", "ratio", "srate", "value_log","value_log_diff", "value_log_srate", "value_log_diff_srate")
...
Per Savings Rate: xt <br>Ratio of actual to predicted Ratio: yt <br>*Since both these series are nonstationary we have transformed them first before the Granger Causality test.* <br> Δxt = log(xt) − log (xt-1) <br> Δyt = log(𝑦t) − log (𝑦t-1) <br> xh = Δxt <br> yh = Δyt <br>
...
#grangertest(value_log_diff ~ value_log_diff_srate, order = 5, data = data_final)

#Dynamic regression
yh <- embed(data_final$ratio,4)
xh <- embed(data_final$value_log_diff_srate,4)
mh <- lm(yh[,1] ~ yh[,2:4] + xh[,3:4])
summary(mh)
...
***Therefore, we can conclude that*** Personal Savings Rate granger causes Ratio.

**xh4** is statistically significant :

For every unit increase in **xh4**, the response variable that is **Ratio is estimated to increase by** **0.7250 units**, holding other predictors constant.
