...
CFF_5_agg_ratio
# CFF_5_agg_ratio Final file for CFF_5 monthly aggregated data national level 


library("writexl")
write.csv(CFF_5_agg_ratio,"CFF_5_agg_ratio.csv")
  ...
  CFF_5_Ratio_CPI<-merge(CFF_5_agg_ratio,CPI, by ="Date")
  CFF_5_Ratio_CPI
  write.csv(CFF_5_Ratio_CPI,"CFF_5_Ratio_CPI.csv")
...
# Reading data
data <- fread("CFF_5_Ratio_CPI.csv", header = T, fill = TRUE)
#head(data)
summary(data)
...
The ratio plot has a lot of fluctuations in the time series plot with a steep dip after 2022.
...
CPI follows an increasing curve after the dip in 2020.
...
#grangertest(ratio ~ value_log_diff_CPI, order = 4, data = data_final)

#Dynamic regression
yh <- embed(data_final$ratio,3)
xh <- embed(data_final$value_log_diff_CPI,3)
...
***Therefore, we can conclude that CPI granger causes ratio of actuals and projected revenue.***

xh3 is statistically significant.

*For every unit increase in **xh3,** the response variable i.e Ratio of actuals to predicted is estimated to increase by **18.01040 units**, holding other predictors constant.*
...
effect1 = 18.0040*(log(115.1) - log(114.1))
...
*For every one unit increase in CPI, ratio changes by 0.15 units.*
