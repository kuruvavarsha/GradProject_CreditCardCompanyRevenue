---
title: "Refi Index vs Actual Revenue -CFF3 "
author: "UC Team"
format: html
editor: visual
---
...
CFF_3_refi_revenue <- merge(CFF_3_agg_revenue,refi_rate , by = "Date")
CFF_3_refi_revenue
write.csv(CFF_3_refi_revenue,"CFF_3_refi_revenue.csv")
```
...
#| include: false
# Reading data
data <- fread("CFF_3_refi_revenue.csv", header = T, fill = TRUE)
...
xh <- embed(data_final$value_log_diff_refi,6)
mh <- lm(yh[,1] ~ yh[,2:6] + xh)
summary(mh)
...
It can be concluded that ***Refinance Index granger causes Revenue***

**xh3 and xh6 are statistically significant**.

For every unit increase in **xh3**, the response variable that is **Revenue** **is estimated to increase by** **0.22990 units**, holding other predictors constant.

For every unit increase in **xh6**, the response variable that is **Revenue** **is estimated to decrease by** **0.21901 units**, holding other predictors constant.
